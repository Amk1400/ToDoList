...
class EntityPostgres(Generic[T]):
    ...
    def remove_entity(self, entity: T, container: List[T], session: Session, parent: Optional[Project] = None) -> None:
        ...
        self._apply_postgres_remove(container, entity, orm_object, session)
    ...
    def update_entity(self, old_entity: T, new_entity: T, container: List[T], session: Session, parent: Optional[Project] = None) -> None:
        ...
        self._update_in_memory_container(container, new_entity, old_entity)

    def _apply_detail_update(self, new_entity, orm_obj):
    ...
    def _apply_postgres_remove(self, container, entity, orm_obj, session):
    ...
    def _apply_postgres_update(self, new_entity, old_entity_orm, session):
        ...
        self._apply_detail_update(new_entity, old_entity_orm)
        session.commit()

    def _update_in_memory_container(self, container, new_entity, old_entity):
        if old_entity.tasks:
